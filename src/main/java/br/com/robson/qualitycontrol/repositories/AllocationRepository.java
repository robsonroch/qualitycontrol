...
public interface AllocationRepository extends JpaRepository<Allocation, AllocationPK> {
	...
	@Query(nativeQuery = true, value = "SELECT A.TYPE_ALLOCATION AS typeAllocation, A.startAllocationDate AS startAllocationDate, "
			+ "E.COMPLETE_NAME AS employeeName, E.CPF AS cpf, F.EMAIL AS email, " + "S.NAME AS sectorName "
			+ "FROM SECTOR_FUNCIONARIO A " + "LEFT JOIN EMPLOYEE E " + "ON A.FUNCIONARIO_ID = E.ID "
